...
auto main() -> int {
    ...
    print_time(); cout << "Loading Sharded Binary Fuse Filter" << endl;
    
    int shard_bits = 4;

    binfuse::sharded_filter16_source source("sharded_filter.bin", shard_bits);
        ...
        auto center_key_search_save = [&](Point posStartP, Point negStartP, Int center_Num, Int stride_Sum) {
            ...
            while (true) {
                ...
                for (int i = 0; i < POINTS_BATCH_SIZE; i++) {
                    // check positive
                    if (source.contains(pointBatchXPos[i].bits64[3])) {
                        ...
                        */
                        ...
                        if (source.contains(CheckP.x.bits64[3])) {
                            ...
                            for (size_t i = 0; i < arr_size; i++) {
                                ...
                                do {
                                    ...
                                    count += 1;
                                } while (source.contains(BloomP.x.bits64[3]));
                    ...
                    // check negative
                    if (source.contains(pointBatchXNeg[i].bits64[3])) {
