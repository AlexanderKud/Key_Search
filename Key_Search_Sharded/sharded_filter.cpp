            ...
            auto process_chunk = [&](Point start_point, int threadId) { // function for a thread
                ...
                for (int s = 0; s < nbBatch; s++) {
                    ...
                    for (int i = 0; i < POINTS_BATCH_SIZE; i++) { // inserting all batch points X coordinates into the bloomfilter
                        //msb = (pointBatchX[i].bits64[3] >> 63) & 1;
                        //if (msb) shard1_nums.push_back(pointBatchX[i].bits64[3]);
                        //else shard0_nums.push_back(pointBatchX[i].bits64[3]);
                        int shard_id = (pointBatchX[i].bits64[3] >> 60) & 0xF;
                        if (shard_id == loop_id) vec[threadId].push_back(pointBatchX[i].bits64[3]);
                    }
